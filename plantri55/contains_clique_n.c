...
static int comparator(const void *p, const void *q){
    return (degree[*(int*)q] - degree[*(int*)p]);
}
...
static int contains_clique_n(int nbtot, int nbop, int doflip){
    ...
    qsort((void*) vert, nv, sizeof(vert[0]), comparator);
    ...
    for (int i = 0; i < nv; i++){
        ...
        for (int j = 0; j < degree[vi]; j++){
            if (out[potential[j]] == 0){
                e = firstedge[potential[j]];
                //printf("e->end: %d\n", e->end);
                for (int k = 0; k < degree[potential[j]]; k++){
                    if (e->end != vi && connected[e->end] && !out[e->end]){
                        if (exists_last(vi, potential[j], e->end, &AM)){
                            return FALSE;
                        }
                    }
                    e = e->next;
