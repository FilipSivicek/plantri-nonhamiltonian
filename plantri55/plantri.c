...
/*static void old_save_repr(int repr[], int size){
    int end = max_repr_size();
    if (size == 1){
        for (int i = 0; i < end; i++){
            all_repr1[repr_found1][i] = repr[i];
        }
        repr_found1++;
    }
    if (size == 2){
        for (int i = 0; i < end; i++){
            all_repr2[repr_found2][i] = repr[i];
        }
        repr_found2++;
    }
    if (size == 3){
        for (int i = 0; i < end; i++){
            all_repr3[repr_found3][i] = repr[i];
        }
        repr_found3++;
    }
}//*/
...
/*static int old_is_new_repr(int repr[], int size){
    if (size == 1){
        for (int i = 0; i < repr_found1; i++){
            if (same_repr(repr, all_repr1[i])){
                return 0;
            }
        }
    }
    if (size == 2){
        for (int i = 0; i < repr_found2; i++){
            if (same_repr(repr, all_repr2[i])){
                return 0;
            }
        }
    }
    if (size == 3){
        for (int i = 0; i < repr_found3; i++){
            if (same_repr(repr, all_repr3[i])){
                return 0;
            }
        }
    }
...
/*static int old_is_vert_canon(int representation[], int size){
...
static int is_canon_a(int vert, int *nbtot){
    repr[0] = MAXE + MAXN + 10;
    ...
    for (int i = 0; i < nv; i++){
        if (degree[i] == 4){
            ...
            for (int j = 0; j < 4; j++){
                if (degree[e->end] == 5){
                    //if (skip_edge_gadget_e(e) == 0 && skip_edge_gadget_e_mirror(e) == 0){
                        if (degree[e->next->end] == 5 && degree[e->next->next->end] == 5){
...
static int is_canon_b(int vert1, int vert2, int *nbtot){
    repr[0] = MAXE + MAXN + 10;
    ...
    for (int i = 0; i < nv; i++){
        if (degree[i] == 4){
            ...
            for (int j = 0; j < 4; j++){
                if (degree[e->end] == 5){
                    //if (skip_edge_gadget_e(e) == 0 && skip_edge_gadget_e_mirror(e) == 0){ //this might not work
                        if (degree[e->next->end] == 5){
                            if (skip_edge_gadget_e(e) == 0){
                                test = testcanon(e, repr, colour);
                                if (test == 2){
                                    return 0;
                                }
                        ...
                        if (degree[e->prev->end] == 5){
                            if (skip_edge_gadget_e_mirror(e) == 0){
                                test = testcanon_mirror(e, repr, colour);
                                if (test == 2){
                                    return 0;
                                }
        ...
        if (degree[i] == 4){
            ...
            for (int j = 0; j < 4; j++){
                if (degree[e->end] == 5){
                    if (degree[e->next->end] == 5){
                        test = testcanon(e, repr, colour);
                        if (test == 2 && skip_edge_gadget_e(e) == 0){
                            return 0;
                        }
                        if (test == 1){
                            num++;
                    ...
                    if (degree[e->prev->end] == 5){
                        test = testcanon_mirror(e, repr, colour);
                        if (test == 2 && skip_edge_gadget_e_mirror(e) == 0){
                            return 0;
                        }
                        if (test == 1){
                            num++;
    ...
    for (i = 0; i < list_length; i++)
      {
        if (skip_gadget_g(startlist[i]) == 0){
            test = testcanon(startlist[i], representation, colour);
            if (test == 1) { numblist[numbs] = startlist[i]; numbs++; }
            else if (test == 2 && skip_edge_gadget_e(startlist[i]) == 0) return 0;
            test = testcanon_mirror(startlist[i], representation, colour);
            if (test == 1) 
              { numblist_mirror[numbs_mirror] = startlist[i]; numbs_mirror++; }
            else if (test == 2 && skip_edge_gadget_e_mirror(startlist[i]) == 0) return 0;
        }
      }
    ...
    for (int A = 0; A < nv; A++){
        ...
        for (int j = 0; j < degree[A]; j++){
            ...
            if (count_fours_with_4neigh_five < 4 && is_valid_extend_d(helper)){
                //if (((degree[helper->invers->next->next->end] == 4) + (degree[helper->next->invers->prev->prev->end] == 4) + 
                //    (degree[helper->prev->end] == 5 && degree[helper->prev->prev->end] == 5)) >= count_fours_with_4neigh_five - 1){
            ...
            if (count_fours_with_3neigh_five < 3 && !count_fours_with_4neigh_five && is_valid_extend_e(helper)){
                //if (((degree[helper->prev->end] == 5) + (degree[helper->next->invers->next->next->end] == 5) + ((degree[helper->invers->next->next->end] == 4) || 
                //    (degree[helper->invers->prev->prev->end] == 4))) >= count_fours_with_3neigh_five){
            ...
            if (count_fours_with_3neigh_five < 4 && count_fours_with_4neigh_five < 4 && is_valid_extend_f(helper)){
            ...
            if (count_fours_with_2neigh_five < 3 && !count_fours_with_4neigh_five && helper->next < helper->next->invers && is_valid_extend_g(helper)){
...
scannarboni(int nbtot, int nbop){
    ...
    *extf[MAXE/3], *extg[MAXE/3], *exth[MAXE/3];
    ...
    for (int i = 0; i < numexta; i++){
        ...
        if (is_canon_a(exta[i]->start, &xnbtot)){scannarboni(xnbtot, 2);}
    ...
    for (int i = 0; i < numextam; i++){
        ...
        if(is_canon_a(extam[i]->start, &xnbtot)){scannarboni(xnbtot, 2);}
    ...
    for (int i = 0; i < numextb; i++){
        ...
        if (is_canon_b(extb[i]->next->end, extb[i]->prev->prev->end, &xnbtot)) scannarboni(xnbtot, 2);
...
narboni_dispatch(void){
    ...
    debugfile1 = fopen("incorrect.txt", "w");
